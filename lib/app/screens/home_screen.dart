...
class _HomeScreenState extends State<HomeScreen> {
  ...
  CompositeNode? _firstVariantNode(CompositeNode node) {
    if (node.type == 'variant' && node.children.isNotEmpty) {
      return node;
    }

    for (final child in node.children) {
      final match = _firstVariantNode(child);
  ...
  CompositeNode? get _selectedRoomVariantNode {
    ...
    return _firstVariantNode(selectedNode);
  }
  ...
  String? get _selectedRoomVariantLabel {
    final decorationId = _editingRoomDecorationId;
    final variantNode = _selectedRoomVariantNode;

    if (decorationId == null ||
        variantNode == null ||
        variantNode.children.isEmpty) {
      return null;
    }
    ...
    final variantName = variantNode.name.trim();
    final childName = activeChild.name.trim();

    if (variantName.isNotEmpty && childName.isNotEmpty) {
      return '$variantName: $childName';
    }
    ...
    if (variantName.isNotEmpty) {
      return '$variantName ${activeIndex + 1}';
    }

    return 'Variant ${activeIndex + 1}';
  }

  void _stepSelectedRoomVariant(int delta) {
    final decorationId = _editingRoomDecorationId;
    final variantNode = _selectedRoomVariantNode;
                        ...
                        child: Padding(
                          ...
                          child: Wrap(
                            ...
                            children: [
                              ...
                              if (_isEditingRoomContents &&
                                  _selectedRoomVariantNode != null) ...[
                                IconButton(
                                  tooltip: 'Previous Variant',
                                  onPressed: () => _stepSelectedRoomVariant(-1),
                                  icon: const Icon(
                                    Icons.chevron_left,
                                    color: Colors.amberAccent,
                                  ),
                                ...
                                ConstrainedBox(
                                  ...
                                  child: Chip(
                                    ...
                                    label: Text(
                                      _selectedRoomVariantLabel ?? 'Variant',
                                      overflow: TextOverflow.ellipsis,
                                    ),
                                    backgroundColor: Colors.black54,
                                    side: const BorderSide(
